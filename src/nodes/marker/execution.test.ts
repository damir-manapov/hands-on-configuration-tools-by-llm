...
describe('Marker Node - Execution', () => {
  it('should execute marker node with equals operator - match', async () => {
    ...
    const result = await markerNodePlugin.execute(node, input);
  ...
  it('should throw error when field is missing', async () => {
    ...
    await expect(markerNodePlugin.execute(node, input)).rejects.toThrow(
